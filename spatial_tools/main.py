...
def main():
    ...
    run(args.output_dir, args.rts_file, args.control_prefix, dragen_file=args.dragen_output, jbruand_file=args.jbruand_output, dnd1_file=args.dnd1_output, dnd2_file=args.dnd2_output)

def run(output, rts_file, rts_prefix, dragen_file=None, jbruand_file=None, dnd1_file=None, dnd2_file=None):
    rts_table = RtsTable.from_file(rts_file, rts_prefix)
    analyzer = RtsAnalyzer(output, rts_table)
